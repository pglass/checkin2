...
// scripts/build-opencv-static.sh (Windows) or scripts/build-opencv-static-darwin.sh (macOS),
// with every flag supplied through the CGO_* environment by scripts/opencv-env.sh.
...
// CGO_LDFLAGS. See scripts/opencv-env.sh, which every build and test entry point
// sources.
