...
// Boolean settings get a checkbox, not a text entry, and the box reflects the
// saved value. Non-boolean settings keep their entry.
func TestSettingsBoolFieldUsesCheckbox(t *testing.T) {
	s, _ := newTestSettings(t)

	var bools, others int
	for _, row := range s.rows {
		isBool := isBoolField(row.field)
		switch {
		case isBool:
			bools++
			if row.check == nil {
				t.Errorf("%s: boolean setting has no checkbox", row.field.Key)
				continue
			}
			if row.entry != nil {
				t.Errorf("%s: boolean setting also built a text entry", row.field.Key)
			}
			want := row.field.Get(config.Default()) == "true"
			if row.check.Checked != want {
				t.Errorf("%s: checked = %v, want %v", row.field.Key, row.check.Checked, want)
			}
		default:
			others++
			if row.entry == nil {
				t.Errorf("%s: non-boolean setting has no entry", row.field.Key)
			}
			if row.check != nil {
				t.Errorf("%s: non-boolean setting built a checkbox", row.field.Key)
			}
		}
	}
	if bools == 0 || others == 0 {
		t.Fatalf("test needs both kinds: %d bool, %d other", bools, others)
	}
}

// Unchecking a boolean writes false; checking it writes true.
func TestSettingsCheckboxSaves(t *testing.T) {
	s, path := newTestSettings(t)

	var row *settingsRow
	for _, r := range s.rows {
		if r.field.Key == "confirm_scan" {
			row = r
		}
	}
	if row == nil {
		t.Fatal("no confirm_scan row")
	}
	if !row.check.Checked {
		t.Fatal("confirm_scan should default to checked")
	}

	row.check.SetChecked(false)
	if !s.restartNote.Visible() {
		t.Error("restart note not shown after toggling a checkbox")
	}
	s.save()
	...
	if got.ConfirmScan {
		t.Error("ConfirmScan = true after unchecking the box, want false")
	}
	if s.app.cfg.ConfirmScan {
		t.Error("live config still has ConfirmScan = true")
	}
}

// Toggling a checkbox back to its original state is not a change.
func TestSettingsCheckboxToggleBackIsNotDirty(t *testing.T) {
	s, _ := newTestSettings(t)
	...
	if row == nil {
		t.Fatal("no checkbox row")
	}

	orig := row.check.Checked
	row.check.SetChecked(!orig)
	if !s.restartNote.Visible() {
		t.Error("restart note not shown after toggling")
	}
	row.check.SetChecked(orig)
	if s.restartNote.Visible() {
		t.Error("restart note still visible after toggling back")
	}
}
