...
// them once after Open would leave any second connection the pool creates
// without them — a lock collision on that connection would fail immediately
// instead of waiting. journal_mode=WAL is
...
func Open(path string) (*Store, error) {
	...
	// not just the first (see connPragmas): any connection the pool opens must
	// have busy_timeout set.
...
// Queries exposes the raw querier for tests and the seed tool.
